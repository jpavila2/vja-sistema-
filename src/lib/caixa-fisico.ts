...
export type EntradaCaixa = {
  saldoInicial: number; saques: number[]; comprasDinheiro: number[]; despesas: number[]; contado?: number;
};
export type SaldoCaixa = {
  totalSaques: number; totalCompras: number; totalDespesas: number; saldoCalculado: number; diferenca: number | null;
};
